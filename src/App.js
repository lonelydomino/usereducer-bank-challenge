...
const reducer = (state, action) => {
  ...
  switch(action.type){
    ...
    case 'payLoan':
      if(state.loan === 5000) return {...state, balance: state.balance - state.loan, loan: 0}
      return {...state}
    case 'closeAccount':
      if( state.balance !== 0 || state.loan === 5000) return {...state}
...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <p>
        <button onClick={() => {dispatch({type: 'payLoan'})}} disabled={!isActive || loan !== 5000}>
          Pay loan
        </button>
      </p>
      <p>
